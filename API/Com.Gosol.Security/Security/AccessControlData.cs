namespace Com.Gosol.VHTT.Security
{
    ...
    internal sealed class AVHTTessControlData
    {
        ...
        //private static string SELECT_ACE_FROM_USER_ID = "Select ObjectTypeID, ObjectID, AVHTTessRight From [User] Inner Join UserGroup On [User].UserID = UserGroup.UserID Inner Join [Group] On UserGroup.GroupID = [Group].GroupID Inner Join ObjectACL On [Group].GroupID = EntityID Where ([Group].Status <> @GroupStatus) Group By ObjectID, ObjectTypeID, AVHTTessRight, [User].UserID Having [User].UserID = @UserID";
        ...
        //private static string SELECT_ACETYPE_FROM_USER_ID = "Select ObjectTypeID, AVHTTessRight From [User] Inner Join UserGroup On [User].UserID = UserGroup.UserID Inner Join [Group] On UserGroup.GroupID = [Group].GroupID Inner Join ObjectTypeACL On [Group].GroupID = EntityID Where ([Group].Status <> @GroupStatus) Group By ObjectTypeID, AVHTTessRight, [User].UserID Having [User].UserID = @UserID";
        ...
        private static Hashtable HashKeyValues(IDbConnection dbConnection, IDbCommand dbCommand, ObjectInfoDelegate objectInfoDelegate, ACLType aclType)
        {
            ...
            {
                throw new AVHTTessControlExceptions("Error at HashKeyValues method in DataAVHTTessManager class. Detail:" + exception.Message);
            }
            finally
            {
                dbConnection.Close();
        ...
        public static void RequestAVHTTessRight(int userId, out Hashtable acl, out Hashtable aclType)
        {
            ...
            //acl = HashKeyValues(dbConnection, dbCommand, new ObjectInfoDelegate(AVHTTessControlData.HashACLKeyValues), ACLType.ObjectInstance);
            acl = new Hashtable();
            dbCommand = DatabaseProxy.CreateDBCommand(dbConnection, SELECT_ACETYPE_FROM_USER_ID);
            DatabaseProxy.AddParameter(dbCommand, PARM_USER_ID, userId);
            aclType = HashKeyValues(dbConnection, dbCommand, new ObjectInfoDelegate(AVHTTessControlData.HashACLKeyValues), ACLType.ObjectClass);
        }

        public static void RequestUserInfo(int userID, out Hashtable userInfo, out Hashtable aVHTTessGroup)
        {
            IDbConnection dbConnection = DatabaseProxy.CreateDBConnection();
            ...
            DatabaseProxy.AddParameter(dbCommand, PARM_LOCKED, Convert.ToInt32(UserStatus.Locked));
            DatabaseProxy.AddParameter(dbCommand, PARM_USER_ID, userID);
            aVHTTessGroup = HashKeyValues(dbConnection, dbCommand, null, ACLType.ObjectList);
        }
        ...
        public static bool VerifyUser(string userName, string password, out int userID)
        {
            ...
            {
                throw new AVHTTessControlExceptions("DatabaseException");
            }
            finally
            {
                dbConnection.Close();
