...
namespace Com.Gosol.VHTT.DAL.HeThong
{
    ...
    public class SystemConfigDAL
    {
        ...
        // Insert
        //private DbVHTTContext _DbVHTTContext;
        //public SystemConfigDAL(DbVHTTContext dbVHTTContext)
        //{
        //    _DbVHTTContext = dbVHTTContext;
        //}
                        ...
                        using (SqlConnection conn = new SqlConnection(SQLHelper.appConnectionStrings))
                        {
                            ...
                            using (SqlTransaction trans = conn.BeginTransaction())
                            {
                                try
                                {
                                    ...
                                    trans.Commit();
                                    Result.Message = ConstantMessage.Alert_Insert_Success("Tham số hệ thống");
                                }
                                catch (Exception ex)
                                {
                                    Result.Status = -1;
                        ...
                        using (SqlConnection conn = new SqlConnection(SQLHelper.appConnectionStrings))
                        {
                            ...
                            using (SqlTransaction trans = conn.BeginTransaction())
                            {
                                try
                                {
                                    ...
                                    trans.Commit();
                                    Result.Message = ConstantMessage.Alert_Update_Success("Tham số hệ thống");
                                }
                                catch
                                {
                                    Result.Status = -1;
        ...
        public BaseResultModel Delete(List<int> ListSystemConfigID)
        {
            ...
            else
            {
                ...
                Result.Status = 1;
                Result.Message = ConstantMessage.Alert_Delete_Success("tham số hệ thống");
                return Result;
            }
        }
