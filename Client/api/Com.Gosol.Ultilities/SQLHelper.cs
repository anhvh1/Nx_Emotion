...
namespace Com.Gosol.VHTT.Ultilities
{
	...
	public class SQLHelper
	{
		...
		/// <returns></returns>
        public static int DeleteOrCheckDataTable(int idTable, int typeTable, bool isDelete)
        {
            int result;
            ...
            try
            {
                ...
                parameters[0].Value = idTable;
                parameters[1].Value = typeTable;

                using (SqlConnection conn = new SqlConnection(appConnectionStrings))
                {
                    conn.Open();
                    using (SqlTransaction trans = conn.BeginTransaction())
                    {
                        try
                        {
                            result = Convert.ToInt32(ExecuteScalar(trans, CommandType.StoredProcedure, isDelete ? "DeleteDataTable" : "CheckDataTable", parameters));
                            trans.Commit();
                        }
                        catch (Exception ex)
                        {
                            Console.WriteLine("Delete Error 1 : " + ex.Message);
                            result = -1;
                            trans.Rollback();
                        }
                    }
            ...
            {
                Console.WriteLine("Delete Error DAL 2 : " + ex.Message);
                throw ex;
            }
        }
